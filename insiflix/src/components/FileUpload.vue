...
<div class="container">
    <form class="add-form" @submit="onSubmit">
        <div class="form-container input-box">
            <DragAndDropUpload @handleFileInput="val => setVideo(val)" :identity="'video'" :required="{ required: true }" :accepted="'.mp4,.webm,.mov,.avi'" :description="'Video hochladen*'" :supported="'Unterstützte Dateiendungen: MP4, WEBM, MOV, AVI'"></DragAndDropUpload>
            <DragAndDropUpload @handleFileInput="val => setThumbnail(val)" :identity="'thumbnail'" :required="{ required: false }" :accepted="'.png,.jpg,.jpeg,.webp'" :description="'Thumbnail hochladen'" :supported="'Unterstützte Dateiendungen: PNG, JPEG, JPG, WEBP'"></DragAndDropUpload>
        </div>
        ...
        <button type="submit" value="Einloggen" class="playbuddne">
